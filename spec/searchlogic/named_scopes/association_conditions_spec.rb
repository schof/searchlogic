...
describe Searchlogic::NamedScopes::AssociationConditions do
  it "should create a named scope" do
    #Company.users_username_like("bjohnson").to_sql.squeeze.should == User.username_like("bjohnson").to_sql.squeeze.merge(:joins => :users)
    Company.users_username_like("bjohnson").to_sql.squeeze.should == Company.joins(:users).where("users.username LIKE ?", "%bjohnson%").to_sql.squeeze
  end
  
  it "should create a deep named scope" do
    Company.users_orders_total_greater_than(10).to_sql.squeeze.should == Company.joins(:users=>:orders).where('orders.total > ?', 10).to_sql.squeeze
  end
  ...
  it "should allow the use of foreign pre-existing named scopes" do
    ...
    Company.users_uname("bjohnson").to_sql.squeeze.should == Company.joins(:users).where("users.username = ?", "bjohnson").to_sql.squeeze
  end
  ...
  it "should allow the use of deep foreign pre-existing named scopes" do
    ...
    Company.users_orders_big_id.to_sql.squeeze.should == Company.joins(:users=>:orders).where(condition).to_sql.squeeze
  end
...
#    Company.users_username_has("bjohnson").to_sql.squeeze.should == Company.joins(:users).where("user.username LIKE ?", "%bjohnson%").to_sql.squeeze
#  end

#  it "should not raise errors for scopes that don't return anything" do
#    User.alias_scope :blank_scope, lambda { |value| }
#    Company.users_blank_scope("bjohnson").to_sql.squeeze.should == {:joins => :users}
#  end
...
#    Company.users_orders_total_greater_than(10).to_sql.squeeze.should == Order.total_greater_than(10).to_sql.squeeze.merge(:joins => {:users => :orders})
#    Company.users_orders_total_greater_than(20).to_sql.squeeze.should == Order.total_greater_than(20).to_sql.squeeze.merge(:joins => {:users => :orders})
#  end
#
...
#    Company.users_username_contains("bjohnson").to_sql.squeeze.should == User.username_contains("bjohnson").to_sql.squeeze.merge(:joins => :users)
#  end
#
#  it "should allow deep aliases" do
#    Company.users_orders_total_gt(10).to_sql.squeeze.should == Order.total_gt(10).to_sql.squeeze.merge(:joins => {:users => :orders})
#  end
#
...
#    Company.users_orders_big_id.to_sql.squeeze.should == {:joins=>[" INNER JOIN \"users\" ON users.company_id = companies.id ", " INNER JOIN \"orders\" ON orders.user_id = users.id "]}
#  end
#
...
#    User.company_users_count_10.to_sql.squeeze.should == {:conditions => "\"companies\".\"users_count\" = 10", :joins => :company}
#  end
#
#  it "should delegate to polymorphic relationships" do
#    Audit.auditable_user_type_name_like("ben").to_sql.squeeze.should == {
...
#    Audit.auditable_user_type_some_type_id_like("ben").to_sql.squeeze.should == {
...
#    Audit.auditable_user_type_company_name_like("company").to_sql.squeeze.should == {
